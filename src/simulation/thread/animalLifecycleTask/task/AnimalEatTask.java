...
public class AnimalEatTask implements Runnable {
    ...
    public void run() {
        ...
        if (countAnimalsStart > 0 && animals.stream().filter(c -> !(c.getName().equals("Caterpillar"))).toList().size() > 0) {
            ...
            System.out.printf("ВСЕ ЖИВОТНЫЕ УМЕРЛИ НА %d ДЕНЬ!", StatisticsTask.getCurrentDay());
            IslandSimulation.getInstance().getExecutorService().shutdown();
            System.exit(0);
        } else {
            System.out.printf("В ЖИВЫХ ОСТАЛИСЬ ТОЛЬКО ГУСЕНИЦЫ НА %d ДЕНЬ!", StatisticsTask.getCurrentDay()); // так как они бесссмертные и не требуют пищи
            IslandSimulation.getInstance().getExecutorService().shutdown();
            System.exit(0);
        }
        latch.countDown();
